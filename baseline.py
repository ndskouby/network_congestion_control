...
def random_policy(obs, step):
    """Random actions."""
    return np.random.randint(0, 5)  # Changed from (0, 3) to (0, 5)

def aimd_like(obs, step):
    """Additive Increase Multiplicative Decrease (TCP-like)."""
    loss_rate = obs[3]  # Index 3 is loss rate
    queue_norm = obs[1]  # Index 1 is normalized queue occupancy
    
    # If seeing loss or high queue, decrease aggressively
    if loss_rate > 0.01 or queue_norm > 0.8:
        return 0  # Large decrease
    else:
        return 3  # Small increase (changed from 2 to 3)

def queue_based(obs, step):
    """Control based on queue occupancy."""
    queue_norm = obs[1]  # Normalized queue occupancy
    
    if queue_norm < 0.3:
        return 3  # Small increase (changed from 2 to 3)
    elif queue_norm > 0.7:
        return 1  # Small decrease (changed from 0 to 1)
    else:
        return 2  # Hold (changed from 1 to 2)

def gradual_increase(obs, step):
    """New: Gradual increase with queue-based backoff."""
    queue_norm = obs[1]
    loss_rate = obs[3]
...
baselines = {
    ...
    'AIMD-like': aimd_like,
    'Queue-based': queue_based,
    'Gradual Increase': gradual_increase,  # ← New policy
}
...
try:
    ...
    print("⚠ Could not load RL model, skipping")
...
ax.axhline(y=10, color='black', linestyle='--', linewidth=1, label='Link Capacity', alpha=0.5)
...
ax.axhline(y=100, color='black', linestyle='--', linewidth=1, label='Queue Capacity', alpha=0.5)
...
print(f"\n📊 Results saved to 'baseline_comparison.png'")
plt.show()
